...
function findFilenameCandidates(spec) {
	...
	const matches = allJs.filter(
		(p) => p.endsWith('/' + base) || p.endsWith('/' + base + '.js'),
	);
	return matches;
}
...
for (const file of allJs) {
	...
	for (const spec of specs) {
		...
		if (res.type === 'bare' || res.type === 'liquid') {
			...
			if (res.type === 'liquid') {
				results.push({
					...
					status: 'liquid',
					candidates: [],
					message: 'Contains Liquid; Jekyll will render this',
				});
			}
			continue;
		}
		if (!res.ok) {
			...
			const candRel = cand.map((c) =>
				normalizePosix(path.relative(path.dirname(file), c)),
			);
			results.push({
				file,
				spec,
...
if (liquid.length > 0) {
	console.log(
		'Note: some imports contain Jekyll/Liquid expressions and cannot be statically checked:',
	);
...
if (missing.length === 0) {
	...
	for (const r of missing) {
		...
		if (r.candidates.length === 0) {
			console.log(
				'  No filename matches found in repo for the imported basename.',
			);
		} else {
			...
			for (const c of r.candidates) {
				...
				console.log(
					`    - ${path.relative(projectRoot, c)}  => suggested import: "${suggestion}"`,
				);
			}
		}
	}
		...
		for (const r of missing) {
			if (r.candidates.length === 1) {
				...
				const relPath = normalizePosix(
					path.relative(path.dirname(r.file), candidate),
				);
				...
				const importRegex = new RegExp(
					`(import[\\s\\S]*?from\\s*)(['"])\s*${oldSpecEsc}\s*\\2`,
					'g',
				);
				const exportRegex = new RegExp(
					`(export[\\s\\S]*?from\\s*)(['"])\s*${oldSpecEsc}\s*\\2`,
					'g',
				);
				const dynRegex = new RegExp(
					`(import\\s*\\(\\s*)(['"])${oldSpecEsc}\\2(\\s*\\))`,
					'g',
				);

				let replaced = false;
				content = content.replace(importRegex, (m, p, q) => {
					replaced = true;
					return p + q + newSpec + q;
				});
				content = content.replace(exportRegex, (m, p, q) => {
					replaced = true;
					return p + q + newSpec + q;
				});
				content = content.replace(dynRegex, (m, p, q, s) => {
					replaced = true;
					return p + q + newSpec + q + s;
				});
				...
				if (replaced) {
					...
					fixes++;
					console.log(
						`Fixed ${path.relative(projectRoot, r.file)}  -> replaced "${r.spec}" with "${newSpec}"`,
					);
				} else {
					console.log(
						`Could not safely replace in ${path.relative(projectRoot, r.file)} (pattern not found), manual fix needed.`,
					);
				}
			} else {
				console.log(
					`Skipping auto-fix for ${path.relative(projectRoot, r.file)} — ${r.candidates.length} candidate files found (manual resolution required).`,
				);
			}
		}
