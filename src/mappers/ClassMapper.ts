...
export class ClassMapper implements IMapper<ClassApiDto, Class> {
    ...
    public map(source: ClassApiDto): Class {
        return {
            ...
            starting_equipment: source.starting_equipment.map(startingEquipment => this.mapStartingEquipment(startingEquipment)),
            starting_equipment_options: source.starting_equipment_options.map(startingEquipmentOption => this.choiceMapper.map(startingEquipmentOption)),
            ...
            saving_throws: source.saving_throws.map(savingThrow => this.baseResourceMapper.map(savingThrow)),
            subclasses: source.subclasses.map(subclass => this.baseResourceMapper.map(subclass))
        };
    }
    ...
    private mapSpellcasting(source?: SpellCastingApiDto): SpellCasting | undefined {
        ...
        return {
            ...
            spellcasting_ability: this.baseResourceMapper.map(source.spellcasting_ability)
        };
    }

    private mapSpellCastingInfo(source: SpellCastingInfoApiDto): SpellCastingInfo {
        return {
            name: source.name,
            desc: source.desc
        };
    ...
    private mapStartingEquipment(source: StartingEquipmentApiDto): StartingEquipment {
        return {
