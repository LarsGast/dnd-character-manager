...
export class SkillProficiencyCheckbox extends HTMLInputElement {
	...
	constructor(skill: Skill) {
		...
		this.checked = globals.activePlayerCharacter.isProficientInSkill(
			this.skill.index,
		);
	...
	handleChange(): void {
		...
		document.dispatchEvent(
			new CustomEvent('skillProficiencyChanged', {
	...
	updateDisplay(event?: CustomEvent): void {
		if (
			...
			this.disabled = globals.activePlayerCharacter.isExpertInSkill(
				this.skill.index,
			);
		}
	}
...
customElements.define('skill-proficiency-checkbox', SkillProficiencyCheckbox, {
	extends: 'input',
});
