...
export class InventoryWeaponAttackBonusCell extends HTMLTableCellElement {
	...
	shouldUpdateDisplay(event?: CustomEvent): boolean {
		...
		return (
			...
			(event.type === 'abilityScoreModifierChanged' &&
				event.detail.ability === (inventoryWeapon as any).ability) ||
			(event.type === 'weaponProficiencyChanged' &&
				event.detail.weapon === this.weapon.index) ||
	...
	getAttackBonusValue(): number {
		const inventoryWeapon: any =
			globals.activePlayerCharacter.inventoryWeapons[this.rowIndex];
		return globals.activePlayerCharacter.getWeaponAttackBonus(
...
customElements.define(
	'inventory-weapon-attack-bonus-cell',
	InventoryWeaponAttackBonusCell,
	{ extends: 'td' },
);
