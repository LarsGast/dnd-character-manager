...
export class InventoryArmorRow extends HTMLTableRowElement {
	...
	connectedCallback(): void {
		...
		this.strengthCell.textContent =
			this.armor.str_minimum === 0 ? '-' : this.armor.str_minimum.toString();
		...
		this.stealthCell.textContent = this.armor.stealth_disadvantage
			? 'Disadvantage'
			: '-';
		...
		this.armorClassCell.textContent = getArmorClassDisplayString(
			this.armor.armor_class,
		);

		// Create cell for effective armor class using the custom element.
		this.effectiveArmorClassCell = new InventoryArmorEffectiveArmorClassCell(
			this.armor,
		);
		...
		this.buttonsCell = new InventoryArmorButtonsCell(
			this.armor,
			this.rowIndex - 1,
		);
...
customElements.define('inventory-armor-row', InventoryArmorRow, {
	extends: 'tr',
});
