...
export class ClassesFeaturesDisplay extends HTMLDetailsElement {
	...
	async updateClassFeaturesDisplay(): Promise<void> {
		...
		if (
			!globals.activePlayerCharacter.classes ||
			globals.activePlayerCharacter.classes.length === 0 ||
			!globals.activePlayerCharacter.classes.some(
				(classLevel) => (classLevel as any).index != 'null',
			)
		) {
...
customElements.define('classes-features-display', ClassesFeaturesDisplay, {
	extends: 'details',
});
