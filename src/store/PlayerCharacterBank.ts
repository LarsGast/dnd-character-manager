...
export class PlayerCharacterBankEntry {
	...
	constructor(data: Partial<PlayerCharacterBankEntry> = {}) {
		...
		this.playerCharacter = data.playerCharacter
			? new PlayerCharacter(data.playerCharacter)
			: PlayerCharacter.getDefault();
...
export class PlayerCharacterBank {
	...
	constructor(data: Partial<PlayerCharacterBank> = {}) {
		...
		this.playerCharacterBankEntries = (
			data.playerCharacterBankEntries ?? []
		).map((entry) => new PlayerCharacterBankEntry(entry));
	}
	...
	static load(): PlayerCharacterBank {
		try {
			const playerCharactersAsString = localStorage.getItem(
				PLAYER_CHARACTER_BANK_KEY,
			);
	...
	removeCharacterFromBank(id: string): void {
		this.playerCharacterBankEntries = this.playerCharacterBankEntries.filter(
			(entry) => entry.id != id,
		);
	}
