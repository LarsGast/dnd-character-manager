...
import { Skill } from './domain/resources/Skill';
...
export class PlayerCharacter {
	...
	setProperty(
		propertyName: string,
		propertyValue: string | number | any[] | null,
	): void {
	...
	getProficiencyBonus(): number {
		const totalLevel = this.classes.reduce(
			(sum, cls) => sum + ((cls as any).level || 0),
			0,
		);
	...
	removeProficiencyInSkill(skillIndex: string): void {
		...
		let proficiencies = this.proficiencies.filter(
	...
	removeProficiencyInWeapon(weaponIndex: string): void {
		...
		let weaponProficiencies = this.weaponProficiencies.filter(
	...
	removeProficiencyInArmor(armorIndex: string): void {
		...
		let armorProficiencies = this.armorProficiencies.filter(
			(armor) => armor !== armorIndex,
		);
